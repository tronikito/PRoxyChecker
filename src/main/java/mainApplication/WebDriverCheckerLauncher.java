...
public class WebDriverCheckerLauncher implements Runnable {
    ...
    public WebDriverCheckerLauncher(ArrayDeque<ProxyItem> proxyList, MainController controller, Label reportLabel, Button delete) {
    ...
    public void run() {
        ...
        Platform.runLater(new Thread(new SetLabel("Checking "+proxySize+" (estimated) " + estimated, reportLabel)));
        ...
        try {
            for (ProxyItem x : proxyList) {
                ...
                timeworked.end();
                Platform.runLater(new Thread(new SetLabel("Checking "+proxyChecked+" of "+proxySize+" in: " + timeworked.resultTime() + " (estimated) " + estimated, reportLabel)));
            ...
            timeworked.end();
            Platform.runLater(new Thread(new SetLabel("Finish "+proxyChecked+" of "+proxySize+" in: " + timeworked.resultTime() + " (estimated) " + estimated, reportLabel)));
